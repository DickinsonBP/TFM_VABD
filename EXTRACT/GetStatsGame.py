...
def getStats(html_doc):
    ...
    local_team_tag = soup.find('span', id="_ctl0_MainContentPlaceHolderMaster_equipoLocalNombre")
    visitante_team_tag = soup.find('span', id="_ctl0_MainContentPlaceHolderMaster_equipoVisitanteNombre")
    local_team = local_team_tag.text.strip() if local_team_tag else ""
    visitante_team = visitante_team_tag.text.strip() if visitante_team_tag else ""
    print("Equipo local:", local_team)
    print("Equipo visitante:", visitante_team)
    ...
    teamHome = [local_team] * len(playersHome)
    teamAway = [visitante_team] * len(playersAway)

    return [playersHome, numHome, init5Home, minHome, ptsHome, t2Home, t3Home, t1Home, rOfHome, rDefHome, assisHome, recHome, perHome, tapFavHome, tapConHome, matHome, falComHome, falSofHome, valHome,teamHome], \
           [playersAway, numAway, init5Away, minAway, ptsAway, t2Away, t3Away, t1Away, rOfAway, rDefAway, assisAway, recAway, perAway, tapFavAway, tapConAway, matAway, falComAway, falSofAway, valAway,teamAway]